...
public class TrophyRenderer implements BlockEntityRenderer<TrophyBlockEntity, TrophyRenderState> {
	...
	public TrophyRenderer(BlockEntityRendererProvider.Context context) {
		this.modelByType = Util.memoize(new Function<>() {
			@Override
			public TrophyBlockModel apply(BossVariant variant) {
				return createTrophyModel(context.entityModelSet(), variant);
			}
		});
	}
	...
	public static TrophyBlockModel createTrophyModel(EntityModelSet set, BossVariant variant) {
		return createTrophyModel(new BiFunction<>() {
			@Override
			public TrophyBlockModel apply(EntityType<?> type, ModelLayerLocation layer) {
				try {
