...
public abstract class BlockModelBuilders extends WoodBlockBuilders {
	...
	public void generateAuroraBlocks() {
		...
		for (int i = 0; i < 16; i++) {
			TFModelTemplates.TINTED_BLOCK.createWithSuffix(TFBlocks.AURORA_BLOCK.get(), "_" + i, TextureMapping.cube(TextureMapping.getBlockTexture(base, "_" + i)), this.modelOutput);
		}

		ModelTemplates.CUBE_ALL.create(base, TextureMapping.cube(new Material(TwilightForestMod.prefix("block/aurora_block_0"))), this.modelOutput);

		List<Identifier> auroraVariants = new ArrayList<>();
		for (int i = 0; i < 16; i++) {
			auroraVariants.add(TwilightForestMod.prefix("block/aurora_block_" + i));
		}
		CustomUnbakedBlockStateModel noiseVaryingModel = new UnbakedNoiseVaryingBlockStateModel(auroraVariants);
		this.blockStateOutput.accept(MultiVariantGenerator.dispatch(base, MultiVariant.of(new SimpleCustomBlockStateModelBuilder(noiseVaryingModel))));
		this.registerSimpleTintedItemModel(base, TwilightForestMod.prefix("block/aurora_block_0"), ItemModelUtils.constantTint(-16711758));
