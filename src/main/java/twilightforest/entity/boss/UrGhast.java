...
public class UrGhast extends BaseTFBoss {
	...
	@Override
	public void knockback(double strength, double xRatio, double zRatio, net.minecraft.world.damagesource.DamageSource source, float damageAmount) {
	...
	private void startTantrum() {
		...
		if (this.level() instanceof ServerLevel serverLevel) {
			LightningBolt lightningbolt = EntityTypes.LIGHTNING_BOLT.create(serverLevel, EntitySpawnReason.MOB_SUMMONED);
	...
	private List<BlockPos> scanForTraps(ServerLevel level) {
		...
		List<BlockPos> positions = stream.map(PoiRecord::getPos)
		...
		for (BlockPos immutable : BlockPos.betweenClosed(center.offset(-48, -48, -48), center.offset(48, 48, 48))) {
			if (level.getBlockState(immutable).is(TFBlocks.GHAST_TRAP) && level.canSeeSky(immutable.above())) {
				positions.add(immutable.immutable());
