...
public class ThrowRiderGoal extends MeleeAttackGoal {
	...
	public boolean canUse() {
		return this.mob.getPassengers().isEmpty() &&
			...
			((TFEntityExtensions) this.mob.getTarget()).getData(() -> TFDataAttachments.YETI_THROWING).getThrowCooldown() <= 0 &&
			super.canUse();
	}
	...
	public void stop() {
		if (!this.mob.getPassengers().isEmpty()) {
			...
			if (rider instanceof Player player) {
				var attachment = ((TFEntityExtensions) player).getData(() -> TFDataAttachments.YETI_THROWING);
