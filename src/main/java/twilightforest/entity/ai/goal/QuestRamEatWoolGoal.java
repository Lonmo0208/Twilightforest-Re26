...
public class QuestRamEatWoolGoal extends Goal {
	...
	@Nullable
	private LivingBlock targetItem = null;
	...
	public boolean canUse() {
		...
		List<LivingBlock> items = this.ram.level().getEntitiesOfClass(LivingBlock.class, this.ram.getBoundingBox().inflate(16.0D), item ->
			(item.onGround() || item.isInWater()) && item.isAlive() &&
				!item.getItemStack().isEmpty() && this.ram.hasLineOfSight(item) && this.ram.isItemTempting(item.getItemStack()));
	...
	public boolean canContinueToUse() {
		return this.ram.isAlive() && !this.navigation.isStuck() && !this.navigation.isDone() &&
			this.targetItem != null && this.targetItem.isAlive() && this.ram.isItemTempting(this.targetItem.getItemStack());
	}
	...
	public void tick() {
		...
		if (!this.ram.level().isClientSide()) {
			if (this.targetItem != null && this.ram.isItemTempting(this.targetItem.getItemStack())) {
				this.ram.getLookControl().setLookAt(this.targetItem, this.ram.getMaxHeadYRot() + 20, this.ram.getMaxHeadXRot());
				if (this.ram.distanceToSqr(this.targetItem.position()) < 6.25D && this.ram.tryAccept(this.targetItem.getItemStack())) {
					//consume the wool
