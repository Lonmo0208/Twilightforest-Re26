...
public class OverlayHandler {
	...
	protected static void registerOverlays(RegisterGuiLayersEvent event) {
		...
		event.registerAbove(VanillaGuiLayers.VEHICLE_HEALTH, TwilightForestMod.prefix("hostile_mount_hunger_bar"), (graphics, partialTicks) -> {
			...
			LocalPlayer player = minecraft.player;
			Gui gui = minecraft.gui;
			if (true && minecraft.gameMode.canHurtPlayer() && player != null && HostileMountEvents.isRidingUnfriendly(player)) {
		...
		event.registerAboveAll(TwilightForestMod.prefix("ore_meter_stats"), (graphics, partialTicks) -> {
			...
			LocalPlayer player = minecraft.player;
			Gui gui = minecraft.gui;
			if (player != null && true && !minecraft.getDebugOverlay().showDebugScreen() && minecraft.gui.screen() == null) {
		...
		event.registerAbove(VanillaGuiLayers.ARMOR_LEVEL, TwilightForestMod.prefix("fortification_shield_count"), (graphics, partialTick) -> {
			...
			LocalPlayer player = minecraft.player;
			Gui gui = minecraft.gui;
			if (player != null && true && (minecraft.gameMode.canHurtPlayer() || TFConfig.showFortificationShieldIndicatorInCreative) && player.hasData(TFDataAttachments.FORTIFICATION_SHIELDS) && player.getData(TFDataAttachments.FORTIFICATION_SHIELDS).shieldsLeft() > 0 && TFConfig.showFortificationShieldIndicator) {
	...
	private static void renderIndicator(Minecraft minecraft, GuiGraphicsExtractor graphics, Gui gui, Player player, int screenWidth, int screenHeight) {
		if (minecraft.options.getCameraType().isFirstPerson() && (minecraft.gameMode.getPlayerMode() != GameType.SPECTATOR || (minecraft.hitResult != null && minecraft.hitResult.getType() != HitResult.Type.MISS)) && minecraft.crosshairPickEntity instanceof QuestRam ram) {
