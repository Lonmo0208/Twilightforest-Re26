...
public final class ConfigComments {
	...
	public static final String FIRST_PERSON_SHIELD_SCEPTER = "Allows the Fortification Shield from the Shield Scepter to render in 1st person view. Only affects the caster; other players still see it in 3rd person.";
	public static final String RAINBOW_LEAVES_STYLE = """
				Controls how Rainbow Oak Leaves calculate their rainbow coloring.
				VANILLA_GRID: The classic original Twilight Forest behavior — a blocky, folded, tileable RGB grid (matches official 1.21.1 NeoForge).
				HSV_SMOOTH: A sleek HSV color-wheel gradient using the same Simplex-noise ripple as the Aurora blocks — smooth red→orange→yellow→green→cyan→blue→violet transitions with no muted browns/greys.""";
