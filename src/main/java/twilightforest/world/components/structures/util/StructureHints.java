...
public interface StructureHints {
	...
	static void addBookInformationStatic(ItemStack book, @Nullable String name, int pageCount) {
		...
		String bookKey = TwilightForestMod.ID + ".book." + key;

		Function<Integer, Filterable<Component>> pageGenerationFunc = index -> Filterable.passThrough(Component.translatable(bookKey + "." + (index + 1)));
		...
		book.set(DataComponents.WRITTEN_BOOK_CONTENT, new WrittenBookContent(
			Filterable.passThrough(bookKey),
			BOOK_AUTHOR,
			3,
			list,
			true
		...
		book.set(DataComponents.CUSTOM_NAME, Component.translatable(bookKey));
