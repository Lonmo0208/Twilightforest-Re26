...
import twilightforest.compat.jei.renderers.FakeLivingBlockHelper;
import twilightforest.compat.jei.renderers.FakeLivingBlockRenderer;
...
public class JEICompat implements IModPlugin {
	...
	public static final IIngredientType<FakeLivingBlock> FAKE_ITEM_ENTITY = () -> FakeLivingBlock.class;
	...
	public void registerIngredients(IModIngredientRegistration registration) {
		...
		registration.register(FAKE_ITEM_ENTITY, Collections.emptyList(), new FakeLivingBlockHelper(), new FakeLivingBlockRenderer(16), FakeLivingBlock.CODEC);
	}
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		registration.addRecipes(DryingCategory.DRYING, manager.getRecipes().stream().filter(h -> h.value().getType() == TFRecipes.DRYING_RECIPE.get()).map(h -> (RecipeHolder<DryingRecipe>) h).filter(holder -> !holder.value().getResult().is(TFItems.STALE_BREAD)).map(RecipeHolder::value).toList());
