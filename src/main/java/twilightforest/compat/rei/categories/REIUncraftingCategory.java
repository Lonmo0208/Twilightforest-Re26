...
public class REIUncraftingCategory implements DisplayCategory<REIUncraftingDisplay> {
	...
	public List<Widget> setupDisplay(REIUncraftingDisplay display, Rectangle origin) {
		...
		List<Ingredient> outputs = new ArrayList<>(display.getRecipe().placementInfo().ingredients()); //Collect each ingredient
		outputs.replaceAll(ingredient -> Ingredient.of(ingredient.items()
			.filter(o -> !o.is(TFItemTags.BANNED_UNCRAFTING_INGREDIENTS))
			.filter(o -> new ItemStack(o.value()).getCraftingRemainder() == null)
			.map(Holder::value)));
