...
public class TFREIClientPlugin implements REIClientPlugin {
	...
	public void registerEntryRenderers(EntryRendererRegistry registry) {
		...
		registry.register(EntityEntryDefinition.ENTITY_TYPE, (entry, last) -> {
			if (entry.getValue() instanceof LivingBlock) {
				return RENDER_CACHE.computeIfAbsent(entry, stack -> new EntityEntryDefinition.LivingBlockRenderer());
			}
	...
	public static Entity createLivingBlock(ItemStack stack) {
		LivingBlock entity = new LivingBlock(EntityType.LIVING_BLOCK, Minecraft.getInstance().level);
