...
public class CicadaBlockEntity extends BlockEntity {
	...
	private void playSong() {
		if (!TFConfig.silentCicadas) {
			this.getLevel().playLocalSound(this.getBlockPos().getX(), this.getBlockPos().getY(), this.getBlockPos().getZ(), TFSounds.CICADA, SoundSource.NEUTRAL, 1.0f, (this.getLevel().getRandom().nextFloat() - this.getLevel().getRandom().nextFloat()) * 0.2F + 1.0F, false);
		}
	}
}
