...
public class EmiTransformationPowderRecipe implements EmiRecipe {
	...
	public List<EmiIngredient> getInputs() {
		...
		SpawnEggItem inputEgg = SpawnEggItem.byId(this.input).map(Holder::value).map(item -> (SpawnEggItem) item).orElse(null);
		SpawnEggItem outputEgg = SpawnEggItem.byId(this.output).map(Holder::value).map(item -> (SpawnEggItem) item).orElse(null);
		if (inputEgg != null) {
