...
public class TFEmiCompat implements EmiPlugin {
	...
	public void register(EmiRegistry registry) {
		...
		for (RecipeHolder<?> holder : manager.getRecipes().stream().filter(holder -> holder.value().getType() == RecipeType.SMITHING && holder.value() instanceof NoTemplateSmithingRecipe).toList()) {
			NoTemplateSmithingRecipe recipe = (NoTemplateSmithingRecipe) holder.value();
			registry.addRecipe(new EmiNoSmithingTemplateRecipe(EmiIngredient.of(recipe.getBase()), EmiIngredient.of(recipe.getAddition()), EmiStack.of(recipe.assemble(new SmithingRecipeInput(ItemStack.EMPTY, ItemStack.EMPTY, ItemStack.EMPTY))), recipe));
		}


		for (RecipeHolder<?> holder : manager.getRecipes().stream().filter(holder -> holder.value().getType() == TFRecipes.DRYING_RECIPE.get()).toList()) {
			DryingRecipe recipe = (DryingRecipe) holder.value();
			if (!recipe.getResult().is(TFItems.STALE_BREAD)) {
				registry.addRecipe(new EmiDryingRecipe(new RecipeHolder<>(holder.id(), recipe)));
			}
		}

		EmiTravellersGearGrindstoneRecipe.register(registry);

		for (RecipeHolder<?> holder : manager.getRecipes().stream().filter(holder -> holder.value().getType() == RecipeType.CRAFTING).toList()) {
