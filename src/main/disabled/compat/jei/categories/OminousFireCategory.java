...
public class OminousFireCategory implements IRecipeCategory<OminousFireRecipe> {
	public static final IRecipeType<OminousFireRecipe> OMINOUS_FIRE = IRecipeType.create(TwilightForestMod.ID, "ominous_fire", OminousFireRecipe.class);
	...
	public void draw(OminousFireRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
		...
		// Block rendering removed for NeoForge 26.1.2 compatibility
	...
	public void setRecipe(IRecipeLayoutBuilder builder, OminousFireRecipe recipe, IFocusGroup focuses) {
		builder.addSlot(RecipeIngredientRole.INPUT, 8, 11)
			.setCustomRenderer(JEICompat.ENTITY_TYPE, this.entityRenderer).add(JEICompat.ENTITY_TYPE, recipe.input());

		SpawnEggItem.byId(recipe.input().type()).map(Holder::value).ifPresent(inputEgg -> {
		...
		builder.addSlot(RecipeIngredientRole.OUTPUT, 76, 11)
			.setCustomRenderer(JEICompat.ENTITY_TYPE, this.entityRenderer).add(JEICompat.ENTITY_TYPE, recipe.output());

		SpawnEggItem.byId(recipe.output().type()).map(Holder::value).ifPresent(outputEgg -> {
			//make it so hovering over the entity shows its name
